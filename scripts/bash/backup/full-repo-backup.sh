...
destination="${NORMORDIS_BACKUP_DIR:-/mnt/normordis-backup/backups/repos/core-ui}"
keep_last=5
...
archive="$destination/normordis-core-ui-$timestamp.zip"
stage="$(mktemp -d "${TMPDIR:-/tmp}/ncu-backup.XXXXXX")"
cleanup() { rm -rf "$stage"; }
trap cleanup EXIT
mkdir -p "$destination"
...
tar -C "$repo_root" -cf - \
  --exclude='./node_modules' --exclude='./.pnpm-store' --exclude='./dist' --exclude='./dist-ssr' \
  --exclude='./.vite' --exclude='./.turbo' \
  --exclude='./.cache' --exclude='./.logs' --exclude='./artifacts' \
  --exclude='./coverage' --exclude='./storybook-static' --exclude='./package' \
  ...
  --exclude='*.pem' --exclude='*.key' --exclude='*.tgz' \
  . | tar -C "$stage" -xf -
...
while IFS= read -r -d '' example_file; do
  ...
  \( -path "$repo_root/.git" -o -path "$repo_root/node_modules" \
    -o -path "$repo_root/.pnpm-store" -o -path "$repo_root/dist" \
    -o -path "$repo_root/dist-ssr" -o -path "$repo_root/.cache" \
    -o -path "$repo_root/.turbo" -o -path "$repo_root/.vite" \
    -o -path "$repo_root/coverage" -o -path "$repo_root/storybook-static" \
    -o -path "$repo_root/package" -o -path "$repo_root/artifacts" \) -prune \
  -o -type f -name '.env.example' -print0)

# Um diretório excluído pode conter marcadores ou configuração versionada
# (por exemplo, package/.gitkeep). Esses ficheiros fazem parte do projeto e
# devem ser restauráveis, mesmo quando os artefactos vizinhos são descartados.
...
if ((keep_last > 0)); then
  mapfile -t old_backups < <(
    find "$destination" -maxdepth 1 -type f -name 'normordis-core-ui-*.zip' -printf '%T@ %p\n' \
