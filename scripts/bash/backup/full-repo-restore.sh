...
backup_dir="${NORMORDIS_BACKUP_DIR:-/mnt/normordis-backup/backups/repos/core-ui}"
backup_file=''
restore_dir=''
list_only=false
rebuild=false
...
Uso: full-repo-restore.sh [opções]
  ...
  --rebuild               executar pnpm install e pnpm run build no fim
...
list_backups() {
  ...
  mapfile -t backups < <(
    find "$backup_dir" -maxdepth 1 -type f -name 'normordis-core-ui-*.zip' -printf '%T@ %p\n' \
      | sort -rn | cut -d' ' -f2-
  )
...
if [[ -z "$backup_file" ]]; then
  [[ -d "$backup_dir" ]] || { printf 'Pasta de backup não encontrada: %s\n' "$backup_dir" >&2; exit 1; }
  backup_file="$(find "$backup_dir" -maxdepth 1 -type f -name 'normordis-core-ui-*.zip' -printf '%T@ %p\n' 2>/dev/null | sort -rn | head -n 1 | cut -d' ' -f2-)"
fi
...
if $rebuild; then
  printf '>>> [RESTORE] A reconstruir dependências e build...\n'
  (cd "$restore_dir" && pnpm install && pnpm run build)
fi
