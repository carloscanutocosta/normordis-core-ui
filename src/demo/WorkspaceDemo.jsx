...
function MockApp({ title, color }) {
  return (
    <div className="p-8">
      <div
        className={`inline-flex items-center gap-2 px-3 py-1 rounded-full text-xs font-medium mb-4 ${color}`}
      >
        Demo
      </div>
      <h1 className="text-2xl font-bold mb-2">{title}</h1>
      <p className="text-muted-foreground text-sm">
        Esta é uma app de demonstração integrada na shell do workspace. Substitui este componente
        pelo conteúdo real da tua app.
      </p>
    </div>
  );
}
...
export default function WorkspaceDemo() {
  const [notifications, setNotifications] = useState([
    {
      id: '1',
      title: 'Novo documento partilhado',
      description: 'João Silva partilhou "Relatório Q1"',
      time: 'há 2 min',
      read: false,
      type: 'info',
    },
    {
      ...
      description: 'Revisão de código aprovada',
      time: 'há 15 min',
      read: false,
      type: 'success',
    },
    {
      id: '3',
      title: 'Reunião em 10 minutos',
      description: 'Standup diário às 09:00',
      time: 'há 1 h',
      read: true,
      type: 'warning',
    },
    {
      ...
      description: 'Exportação PDF falhou — tenta novamente',
      time: 'há 2 h',
      read: true,
      type: 'error',
    },
