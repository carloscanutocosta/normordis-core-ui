...
export default function Sparkline({ data = [], dataKey = "value", color, width = 80, height = 32, showTrend = false, className }) {
  const values = data.map((d) => (typeof d === "number" ? d : d[dataKey] ?? 0));
  const trend = values.length > 1 ? values[values.length - 1] - values[0] : 0;
  const auto = trend > 0 ? "hsl(var(--chart-2))" : trend < 0 ? "hsl(var(--destructive))" : "hsl(var(--muted-foreground))";
