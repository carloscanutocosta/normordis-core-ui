...
export default function BarChartComponent({ data = SAMPLE, dataKeys, xKey = "dept", height = 280, horizontal = false, stacked = false, className }) {
  const keys = dataKeys || Object.keys(data[0] || {}).filter((k) => k !== xKey);
  return (
    <div className={cn("w-full", className)}>
      <ResponsiveContainer width="100%" height={height}>
        <ReBarChart data={data} layout={horizontal ? "vertical" : "horizontal"} margin={{ top:5, right:20, left:0, bottom:5 }}>
          <CartesianGrid strokeDasharray="3 3" stroke="hsl(var(--border))" />
          {horizontal
            ? <><XAxis type="number" tick={{ fontSize:12, fill:"hsl(var(--muted-foreground))" }} axisLine={false} tickLine={false} />
                <YAxis type="category" dataKey={xKey} tick={{ fontSize:12, fill:"hsl(var(--muted-foreground))" }} axisLine={false} tickLine={false} width={70} /></>
          ...
          {keys.map((k, i) => (
            <Bar key={k} dataKey={k} stackId={stacked ? "a" : undefined} fill={PALETTE[i % PALETTE.length]} radius={stacked ? 0 : [3,3,0,0]} />
          ))}
        </ReBarChart>
      </ResponsiveContainer>
    </div>
