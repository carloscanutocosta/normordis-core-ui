...
const listColumns = [
  ...
  { key: 'role', label: 'Função' },
  {
    ...
    render: (v) => (
      <Badge
        className={v === 'Ativo' ? 'bg-green-100 text-green-700' : 'bg-muted text-muted-foreground'}
      >
        {v}
      </Badge>
    ),
...
const tableColumns = [
  ...
  },
  {
    key: 'status',
    label: 'Estado',
    inlineOptions: ['Disponível', 'Esgotado'],
    render: (v) => (
      <Badge
        className={
          v === 'Disponível' ? 'bg-green-100 text-green-700' : 'bg-amber-100 text-amber-700'
        }
      >
        {v}
      </Badge>
...
const tableRows = Array.from({ length: 23 }, (_, i) => ({
  ...
  price: [89.99, 549.0, 39.99, 79.99, 129.99, 49.99, 109.99, 159.99][i % 8],
...
const timelineVertical = [
  {
    ...
    description: 'O pedido foi registado no sistema.',
    status: 'completed',
  ...
  },
  {
    id: 2,
    label: 'Em análise',
    description: 'A equipa está a validar os dados.',
    status: 'completed',
    timestamp: '01/05/2026 10:30',
  },
  {
    id: 3,
    ...
    status: 'active',
    timestamp: '01/05/2026 11:15',
  },
  {
    id: 4,
    label: 'Processamento',
    description: 'Será processado após aprovação.',
    status: 'pending',
    timestamp: null,
  },
  {
...
const timelineWithError = [
  ...
  { id: 2, label: 'Autenticação', status: 'completed', timestamp: '09:01' },
  {
    ...
    label: 'Transferência',
    status: 'error',
    description: 'Falha na ligação ao servidor.',
    timestamp: '09:03',
  },
...
const tabItems = [
  {
    id: 'overview',
    label: 'Visão Geral',
    content: 'Resumo geral do projecto com métricas e indicadores principais.',
    icon: BarChart2,
  },
  {
    id: 'reports',
    label: 'Relatórios',
    content: 'Lista de relatórios gerados automaticamente pela plataforma.',
    icon: FileText,
  },
  {
    id: 'users',
    label: 'Utilizadores',
    content: 'Gestão de utilizadores, permissões e grupos de acesso.',
    icon: Users,
  },
  {
    id: 'settings',
    label: 'Definições',
    content: 'Configurações gerais da conta, integrações e preferências.',
    icon: Shield,
  },
];
...
export default function DataSection() {
  ...
  const openModal = (mode, record, fields, title) => setModal({ record, fields, mode, title });
    ...
    <div className="space-y-8">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Cartões de métricas com ícone, valor e indicador de tendência
          </CardDescription>
        </CardHeader>
        <CardContent>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Banners de feedback — informação, sucesso, aviso, erro (com dismiss)
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-3">
          <AlertBanner
            ...
            description="Actualize a aplicação para aceder às últimas funcionalidades."
            dismissible
          />
          <AlertBanner
            variant="success"
          ...
          <AlertBanner
            variant="warning"
            title="Atenção"
            description="O período de avaliação termina em 3 dias."
            dismissible
          />
          <AlertBanner
            variant="error"
            title="Erro de ligação"
            description="Não foi possível ligar ao servidor. Tente novamente."
            dismissible
          />
        </CardContent>
      </Card>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Visualização de etapas de um procedimento — vertical, horizontal e com erro
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-10">
          <div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Hamburger, Kebab (⋮), Meatball (⋯), Dropdown com submenu, Speed Dial e Context Menu
          </CardDescription>
        </CardHeader>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Tabela com pesquisa, ordenação, paginação e painel de filtros avançados
          </CardDescription>
        </CardHeader>
        <CardContent>
          <DataTable
