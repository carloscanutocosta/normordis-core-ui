...
const NAV_LINKS = [
  ...
  { label:"Preços",      href:"#" },
  { label:"Documentação",href:"#", children:["Guia de início","Referência API","Exemplos","Changelog"] },
  { label:"Empresa",     href:"#" },
];
    ...
    <div className={cn("rounded-xl overflow-hidden border border-border", className)}>
      <nav className="bg-card border-b border-border">
        <div className="flex items-center justify-between px-4 h-14">
            ...
            {NAV_LINKS.map((link) => (
              <div key={link.label} className="relative" onMouseEnter={() => setDropdown(link.label)} onMouseLeave={() => setDropdown(null)}>
                <button className="flex items-center gap-1 px-3 py-1.5 text-sm text-foreground hover:text-primary rounded-md hover:bg-muted transition-colors">
                  {link.label}
                  {link.children && <ChevronDown className={cn("h-3.5 w-3.5 transition-transform", dropdown === link.label && "rotate-180")} />}
                </button>
                {link.children && dropdown === link.label && (
                  <div className="absolute top-full left-0 mt-1 bg-card border border-border rounded-lg shadow-lg p-1 w-44 z-20">
                    {link.children.map((c) => (
                      <button key={c} className="w-full text-left px-3 py-1.5 text-sm text-foreground hover:bg-muted rounded-md transition-colors">{c}</button>
                    ))}
                  </div>
                )}
              </div>
          ...
          {/* Right actions */}
          <div className="flex items-center gap-2">
            <button className="p-2 rounded-lg hover:bg-muted transition-colors hidden md:flex"><Search className="h-4 w-4 text-muted-foreground" /></button>
            <button className="p-2 rounded-lg hover:bg-muted transition-colors hidden md:flex"><Bell className="h-4 w-4 text-muted-foreground" /></button>
        ...
        {mobile && (
          <div className="md:hidden border-t border-border p-3 space-y-1">
            {NAV_LINKS.map((link) => (
              <button key={link.label} className="w-full text-left px-3 py-2 text-sm text-foreground hover:bg-muted rounded-md transition-colors">{link.label}</button>
            ))}
          </div>
        )}
      </nav>
