...
const TAG_COLORS = { Design:"bg-purple-100 text-purple-700", Bug:"bg-red-100 text-red-700", Feature:"bg-blue-100 text-blue-700", Backend:"bg-indigo-100 text-indigo-700", QA:"bg-teal-100 text-teal-700", Review:"bg-amber-100 text-amber-700", DevOps:"bg-orange-100 text-orange-700", Docs:"bg-slate-100 text-slate-600" };
      ...
      {(provided, snapshot) => (
        ...
        >
          ...
          <div className="flex items-center gap-1.5 flex-wrap">
            <span className={cn("text-[10px] font-medium px-1.5 py-0.5 rounded", TAG_COLORS[card.tag] || "bg-muted text-muted-foreground")}>{card.tag}</span>
            <span className={cn("text-[10px] font-medium px-1.5 py-0.5 rounded ml-auto", PRIORITY_COLORS[card.priority])}>{card.priority}</span>
          </div>
        </div>
      )}
    </Draggable>
...
export default function KanbanBoard({ className }) {
  ...
  const onDragEnd = ({ source, destination, draggableId }) => {
    ...
    if (srcCol === dstCol) {
      ...
      setState((s) => ({ ...s, columns: { ...s.columns, [srcCol.id]: { ...srcCol, cardIds: ids } } }));
    } else {
      ...
      setState((s) => ({
        ...s,
        columns: { ...s.columns, [srcCol.id]: { ...srcCol, cardIds: srcIds }, [dstCol.id]: { ...dstCol, cardIds: dstIds } },
      }));
    }
  };
    ...
    <DragDropContext onDragEnd={onDragEnd}>
      <div className={cn("flex gap-4 overflow-x-auto pb-4", className)}>
        {state.columnOrder.map((colId) => {
          ...
          const colors = COLORS[colId] || COLORS.todo;
          return (
            <div key={colId} className="flex-shrink-0 w-64 flex flex-col gap-2">
              <div className={cn("flex items-center gap-2 px-3 py-2 rounded-lg font-medium text-sm", colors.header)}>
              ...
              <Droppable droppableId={colId}>
                {(provided, snapshot) => (
                  <div
                    ...
                    className={cn("flex flex-col gap-2 min-h-[120px] rounded-lg p-2 transition-colors", snapshot.isDraggingOver && "bg-primary/5 ring-1 ring-primary/20")}
                  >
              ...
              <Button variant="ghost" size="sm" className="text-muted-foreground gap-1.5 justify-start">
