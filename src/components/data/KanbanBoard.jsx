        ...
        >
          ...
          {(card.tag || card.priority) && (
            <div className="flex items-center gap-1.5 flex-wrap">
              {card.tag && (
                <span className={cn("text-[10px] font-medium px-1.5 py-0.5 rounded", TAG_COLORS[card.tag] || "bg-muted text-muted-foreground")}>
                  {card.tag}
                </span>
              )}
              {card.priority && (
                <span className={cn("text-[10px] font-medium px-1.5 py-0.5 rounded ml-auto", PRIORITY_COLORS[card.priority] || "bg-muted text-muted-foreground")}>
                  {card.priority}
                </span>
              )}
            </div>
...
export default function KanbanBoard({
  ...
  const onDragEnd = ({ source, destination, draggableId }) => {
    ...
    if (srcCol === dstCol) {
      ...
      applyState({ ...state, columns: { ...state.columns, [srcCol.id]: { ...srcCol, cardIds: ids } } });
    } else {
