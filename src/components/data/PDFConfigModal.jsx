    ...
    preview: (
      <div className="w-full h-full bg-white rounded overflow-hidden text-[4px] font-mono">
        ...
        <div className="px-1 pt-0.5 space-y-0.5">
          ...
          {[0,1,2,3].map(i => (
            <div key={i} className={cn("h-1 w-full rounded-sm", i%2===0?"bg-gray-50":"bg-white")} />
          ))}
        </div>
    ...
    preview: (
      <div className="w-full h-full bg-gray-900 rounded overflow-hidden">
        ...
        <div className="px-1 pt-0.5 space-y-0.5">
          ...
          {[0,1,2,3].map(i => (
            <div key={i} className={cn("h-1 w-full rounded-sm", i%2===0?"bg-gray-800":"bg-gray-750")} />
          ))}
        </div>
      </div>
    ),
  ...
  return (
    <div className="rounded-md border border-input overflow-hidden">
      ...
      <div className="flex items-center justify-between px-3 py-1.5 border-t border-border bg-muted/30">
        ...
        <Button type="button" variant="ghost" size="sm" onClick={clear} disabled={isEmpty} className="h-6 text-xs gap-1 text-muted-foreground">
...
export function buildChartImage(rows, columns) {
  ...
  // Bars
  const COLORS = ["#4338ca","#059669","#d97706","#dc2626","#7c3aed","#0891b2","#be185d","#b45309"];
...
async function generatePDF({ rows, columns, filename, reportTitle, template, signature, chartDataUrl }) {
  ...
  // ── TEMPLATE STYLES ────────────────────────────────────────────────────
  const styles = {
    corporate: { primary: [67, 56, 202], headerText: [255,255,255], stripBg: [243,244,246], rowAlt: [248,248,252], border: [200,200,215], text: [30,30,50], footerBg: [243,244,246] },
    minimal:   { primary: [30, 30, 30],  headerText: [255,255,255], stripBg: [248,248,248], rowAlt: [250,250,250], border: [220,220,220], text: [30,30,30],  footerBg: [248,248,248] },
    dark:      { primary: [30, 30, 46],  headerText: [200,200,255], stripBg: [40, 40, 58],  rowAlt: [36, 36, 52],  border: [60, 60, 80],  text: [220,220,240], footerBg: [30,30,46] },
  };
  ...
  if (chartDataUrl) {
    ...
    doc.text(`Gráfico resumo — ${rows.length} registo(s) | Gerado em: ${time}`, marginX, 24 + chartH_mm + 6);
  ...
  const drawPageHeader = (isFirst) => {
    ...
    if (isFirst) {
      ...
      doc.text(`Template: ${template} | Gerado em: ${time}`, pageW - marginX, 28.5, { align: "right" });
    } else {
  ...
  const addFooter = (isLast) => {
    ...
    doc.text(`Página ${doc.internal.getCurrentPageInfo().pageNumber}`, pageW - marginX, pageH - 3.5, { align: "right" });
  };
  ...
  rows.forEach((row, rowIdx) => {
    ...
    doc.setFontSize(8);
    columns.forEach((col, i) => {
      doc.text(String(row[col.key] ?? "—"), marginX + i * colW + 3, y + 5.5, { maxWidth: colW - 4 });
    });
...
// ── Modal component ────────────────────────────────────────────────────────
export default function PDFConfigModal({ open, onClose, rows, allRows, columns, filename, reportTitle }) {
  ...
  const handleGenerate = async () => {
    ...
    await generatePDF({ rows: data, columns: exportCols, filename, reportTitle, template, signature: signature || null, chartDataUrl });
    setLoading(false);
    onClose();
  };
      ...
      <div className="relative bg-card border border-border rounded-xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-7">
              ...
              {[
                ...
                <button
                  ...
                  className={cn(
                    ...
                    scope === id ? "border-primary bg-primary/5 text-primary font-medium" : "border-border text-foreground hover:border-primary/40",
                    id === "filtered" && !isFiltered && "opacity-40 cursor-not-allowed"
                  )}
                >
                  <span>{label}</span>
                  <span className={cn("text-xs font-semibold px-2 py-0.5 rounded-full", scope === id ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground")}>
                    {count}
                  </span>
                </button>
              ))}
          ...
          <section className="space-y-3">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-sm font-semibold text-foreground flex items-center gap-1.5"><LayoutTemplate className="h-4 w-4" /> Template de layout</h3>
            </div>
              ...
              {TEMPLATES.map((tmpl) => (
                <button
                  ...
                  className={cn(
                    ...
                    template === tmpl.id ? "border-primary ring-2 ring-primary/20" : "border-border hover:border-primary/40"
                  )}
                >
                  <div className="relative h-20 bg-muted/30 p-1.5">
                    {tmpl.preview}
                  </div>
                  <div className="px-2.5 py-2 border-t border-border bg-card">
                    <p className={cn("text-xs font-semibold", template === tmpl.id ? "text-primary" : "text-foreground")}>{tmpl.label}</p>
                    <p className="text-[10px] text-muted-foreground leading-tight mt-0.5">{tmpl.description}</p>
                  </div>
                </button>
              ))}
            </div>
          ...
          <section className="space-y-3">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-sm font-semibold text-foreground flex items-center gap-1.5"><BarChart2 className="h-4 w-4" /> Gráfico resumo</h3>
            </div>
            <label className="flex items-start gap-3 rounded-lg border border-border p-3 cursor-pointer hover:bg-muted/20 transition-colors">
              <div className="relative mt-0.5">
                <input
                  ...
                  className="sr-only"
                />
                <div className={cn("h-4 w-4 rounded border flex items-center justify-center transition-colors", includeChart ? "bg-primary border-primary" : "border-input")}>
                  {includeChart && <svg className="h-2.5 w-2.5 text-primary-foreground" fill="none" viewBox="0 0 10 10"><path d="M1.5 5l2.5 2.5 4.5-4" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/></svg>}
                </div>
              </div>
              <div>
                <p className="text-sm font-medium text-foreground">Incluir gráfico de barras resumo</p>
                <p className="text-xs text-muted-foreground mt-0.5">Gerado automaticamente a partir da primeira coluna numérica dos dados seleccionados. Inserido antes da tabela.</p>
              </div>
            </label>
          </section>
          ...
          <section className="space-y-3">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-sm font-semibold text-foreground flex items-center gap-1.5"><PenLine className="h-4 w-4" /> Assinatura digital (opcional)</h3>
            </div>
            <p className="text-xs text-muted-foreground">Se preenchida, a assinatura será inserida na última página, na secção de rodapé, com data/hora de assinatura.</p>
        ...
        <div className="flex items-center justify-between px-6 py-4 border-t border-border shrink-0 bg-muted/20">
          <div className="text-xs text-muted-foreground">
            {data.length} registo(s) · template <strong>{template}</strong>{signature && " · c/ assinatura"}{includeChart && " · c/ gráfico"}
          </div>
          <div className="flex items-center gap-2">
            <Button variant="outline" size="sm" onClick={onClose}>Cancelar</Button>
            <Button size="sm" className="gap-2 min-w-[130px]" onClick={handleGenerate} disabled={loading}>
              {loading ? <><Loader2 className="h-4 w-4 animate-spin" /> A gerar…</> : <><FileDown className="h-4 w-4" /> Gerar PDF</>}
            </Button>
          </div>
        </div>
