...
import { ChevronDown, Home, BarChart2, Users, Settings, FileText, Shield, Bell, HelpCircle } from "lucide-react";
      ...
      {hasChildren && open && (
        <div className="mt-0.5 space-y-0.5 border-l border-border ml-5 pl-1">
          {item.children.map(child => (
            <MenuItem key={child.id} item={child} depth={depth + 1} activeId={activeId} onSelect={onSelect} openId={openId} onToggle={onToggle} />
          ))}
        </div>
      )}
    </div>
...
export default function AccordionMenu({ className }) {
  ...
  return (
    <nav className={cn("w-full space-y-0.5", className)}>
      {MENU_ITEMS.map(item => (
        <MenuItem key={item.id} item={item} activeId={activeId} onSelect={setActiveId} openId={openId} onToggle={setOpenId} />
      ))}
    </nav>
  );
}