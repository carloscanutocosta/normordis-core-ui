    ...
    return (
      <div className="flex flex-col items-center flex-1">
        ...
        <div className="flex items-center w-full">
          {/* left line */}
          <div className={cn("flex-1 h-0.5", step.status === "completed" ? "bg-primary" : "bg-border")} />
          <div className={cn(
          ...
          {/* right line */}
          {!isLast && <div className={cn("flex-1 h-0.5", step.status === "completed" ? "bg-primary" : "bg-border")} />}
          {isLast && <div className="flex-1" />}
        </div>
        {/* Label */}
        <div className="mt-2 text-center px-1">
          <p className={cn("text-xs font-medium", step.status === "active" ? "text-primary" : step.status === "error" ? "text-destructive" : "text-foreground")}>{step.label}</p>
          {step.description && <p className="text-xs text-muted-foreground mt-0.5">{step.description}</p>}
          {step.timestamp && <p className="text-xs text-muted-foreground/70 mt-0.5">{step.timestamp}</p>}
        </div>
      </div>
    );
  }
    ...
    <div className="flex gap-3">
      ...
      <div className={cn("pb-6", isLast && "pb-0")}>
        <p className={cn(
          ...
          step.status === "active" ? "text-primary" : step.status === "error" ? "text-destructive" : "text-foreground"
        )}>
          {step.label}
        </p>
        {step.description && <p className="text-sm text-muted-foreground mt-0.5">{step.description}</p>}
        {step.timestamp && <p className="text-xs text-muted-foreground/70 mt-1">{step.timestamp}</p>}
      </div>
    </div>
  );
}

export default function Timeline({ steps = [], orientation = "vertical", className }) {
  return (
    <div className={cn(
