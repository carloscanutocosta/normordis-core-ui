...
export default function DragDropList({ initialItems = SAMPLE, onReorder, removable = true, className }) {
  ...
  return (
    <DragDropContext onDragEnd={onDragEnd}>
      <Droppable droppableId="list">
        {(provided) => (
          <div ref={provided.innerRef} {...provided.droppableProps} className={cn("space-y-1.5", className)}>
            {items.map((item, index) => (
              <Draggable key={item.id} draggableId={item.id} index={index}>
                {(prov, snapshot) => (
                  ...
                  >
                    <span {...prov.dragHandleProps} className="text-muted-foreground cursor-grab active:cursor-grabbing">
                      <GripVertical className="h-4 w-4" />
                    </span>
                    <span className="flex-1">{item.text}</span>
                    <span className="text-xs text-muted-foreground/50 tabular-nums w-5 text-right">{index + 1}</span>
                    {removable && (
