...
export default function CodeBlock({
  ...
  return (
    <div
      className={cn('rounded-xl overflow-hidden border border-border font-mono text-sm', className)}
    >
      {/* Title bar */}
      <div className="flex items-center justify-between px-4 py-2.5 bg-[#1e1e2e] border-b border-white/10">
        <div className="flex items-center gap-3">
          ...
          <span className={cn('text-[10px] font-semibold uppercase tracking-wider', langColor)}>
            {language}
          </span>
        </div>
        <button
          ...
          <tbody>
            {lines.map((line, i) => (
              <tr key={i} className="hover:bg-white/5">
                ...
                <td className="px-4 py-0.5 text-white/90 text-xs whitespace-pre align-top">
                  {line || ' '}
                </td>
              </tr>
            ))}
