...
const PRESETS = {
  empty: {
    icon: Inbox,
    title: 'Sem dados',
    description: 'Ainda não existem registos. Comece por adicionar um.',
  },
  search: {
    icon: Search,
    title: 'Sem resultados',
    description: 'A pesquisa não retornou resultados. Tente termos diferentes.',
  },
  error: {
    icon: AlertCircle,
  ...
  noFiles: {
    icon: FileX,
    title: 'Sem ficheiros',
    description: 'Ainda não foram carregados ficheiros.',
  },
  noUsers: {
    icon: Users,
...
interface EmptyStateProps {
  preset?: 'empty' | 'search' | 'error' | 'noFiles' | 'noUsers';
...
export default function EmptyState({
  ...
  const cfg: Partial<(typeof PRESETS)[keyof typeof PRESETS]> = preset ? PRESETS[preset] : {};
  ...
  return (
    ...
    >
      <div
        className="flex items-center justify-center rounded-full bg-muted mb-4"
