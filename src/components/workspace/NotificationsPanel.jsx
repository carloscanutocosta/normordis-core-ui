 ...
 * @param {Function} onNotificationRead  - (id: string) => void
 * @param {Function} onNotificationsReadAll - () => void
 * @param {Function} onNotificationClear - (id?: string) => void  — omit id to clear all
 */
      ...
      <PopoverContent align="end" className="w-80 p-0" sideOffset={8}>
        ...
        <div className="flex items-center justify-between px-3 py-2.5 border-b border-border">
          ...
          <div className="flex items-center gap-1">
            {unreadCount > 0 && onNotificationsReadAll && (
              <Tooltip>
                <TooltipTrigger asChild>
                  <Button variant="ghost" size="icon" className="h-6 w-6" onClick={onNotificationsReadAll} aria-label="Marcar todas como lidas">
            ...
            {notifications.length > 0 && onNotificationClear && (
              <Tooltip>
                <TooltipTrigger asChild>
                  <Button variant="ghost" size="icon" className="h-6 w-6 text-muted-foreground hover:text-destructive" onClick={() => onNotificationClear()} aria-label="Limpar todas">
                  ...
                  >
                    ...
                    <div className="flex flex-col gap-1 shrink-0">
                      {!n.read && onNotificationRead && (
                        <Tooltip>
                          <TooltipTrigger asChild>
                            <Button variant="ghost" size="icon" className="h-5 w-5" onClick={() => onNotificationRead(n.id)} aria-label="Marcar como lida">
                      ...
                      {onNotificationClear && (
                        <Tooltip>
                          <TooltipTrigger asChild>
                            <Button variant="ghost" size="icon" className="h-5 w-5 text-muted-foreground hover:text-destructive" onClick={() => onNotificationClear(n.id)} aria-label="Remover">
